          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for='spuSaleAttr in spuSaleAttrList' :key='spuSaleAttr.id'>
                ...
                <dd changepirce="0" :class="{active:item.isChecked==='1'?true:false}" 
                @click='selectSku(spuSaleAttr.spuSaleAttrValueList,item)' 
                v-for='item in spuSaleAttr.spuSaleAttrValueList' 
                :key="item.id">{{item.saleAttrValueName}}</dd>
              </dl>
             
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" v-model="goodsNum" @change='goodsNum=goodsNum<=1?1:Math.round(goodsNum)' class="itxt">
                <a href="javascript:" class="plus" @click='goodsNum+=1'>+</a>
                <a href="javascript:" class="mins" @click='goodsNum<=1?1:goodsNum-=1'>-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click='skipAddCartSuccess'>加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      // 在页面加载完成后发送请求 一般发送请求都是在mounted阶段
      // 将发送请求封装成一个函数 方便重复调用
      this.getDetailInfo()
    },
    methods:{
      ...
      selectSku(spuSaleAttrValueList,item){
      ...
      // 这里只能使用编程式导航跳转路由
      async skipAddCartSuccess(){
        const obj = {
          skuId:this.skuId,
          goodsNum:this.goodsNum
        }
       try {
          await this.$store.dispatch('getAddCartSuccess',obj)
          sessionStorage.setItem('addCartsSuccsee',JSON.stringify(this.skuInfo))
          this.$router.push('/addCartsSuccess?goodsNum='+this.goodsNum)
       } catch (error) {
          alert(error)
       }
